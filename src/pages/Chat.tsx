...
import { ArrowLeft, Send, Plus, Loader2, Settings, MessageSquare } from 'lucide-react';
  ...
  const sendMessage = async () => {
    ...
    try {
      if (electron?.chat) {
        ...
        setMessages(prev => [...prev, { id: crypto.randomUUID(), role: 'assistant', content: response.content || response.message || 'No response', createdAt: new Date() }]);
      } else {
        await new Promise(r => setTimeout(r, 1000));
        setMessages(prev => [...prev, { id: crypto.randomUUID(), role: 'assistant', content: `Mock response. Run in Electron with API keys for real AI.\n\nYou said: "${userMessage.content}"`, createdAt: new Date() }]);
      }
    } catch (e: any) { toast({ title: 'Error', description: e.message || 'Failed to send', variant: 'destructive' }); }
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="border-t p-4">
          <div className="max-w-3xl mx-auto flex gap-2">
            <Textarea value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKeyDown} placeholder="Type a message..." className="min-h-[60px] resize-none" disabled={isLoading} />
